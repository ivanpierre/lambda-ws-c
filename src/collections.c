...
{
	ASSERT_TYPE(LIST | ARRAY | MAP | SET,
	...
	{
		if(node->val.coll.node[i]])
		{
			free_node(node->val.coll.node[i]);
			node->val.coll.node[i] = NULL;
		}
	}
	return NULL;
}
...
static char **get_array(Node *)
{
	...
	for(long i = 0; i < size; i++)
	{
		Node *curr = node->val.coll.node[i];
		Node *str_node;
...
static long list_size(long size, char **strings)
{
	long res = 0;
	for(long i = 0; i < size; i++)
		res += strings[i] + 1; // counting space
	return res + 1; // for \0
}
...
static String string_coll_elements(Node *node)
{
	...
	for(long i = 0; i < size; i++)
	{
		...
		if(i < size - 1)
			strcat(string_res, " ");
		free(str_size[i]); // free each allocated string node
	}
...
static String string_coll_elements_reverse(Node *node)
{
	...
	for(long i = size - 1; i >= 0; i--)
	{
		...
		if(i < size - 1)
			strcat(string_res, " ");
		free(str_size[i]); // free each allocated string
	}
...
{
	ASSERT(coll, "string_keyval : null pointer");
	ASSERT_TYPE(KEYVAL,
...
{
	// get back inner content
	String inner_content = get_inner_content_coll(Node *coll);
	if(!inner_content)
...
Node *first_coll(Node *coll)
{
	...
	if(coll->type & NIL_NODE ||
	   coll->val.coll.size == 0)
		return nil_node;
	if(coll->type & LIST)
		return link_node(coll->val.coll[coll->val.coll.size - 1]]);
	else
		return link_node(coll->val.coll[0]]);
}
...
Node *nth(Node *coll, long index)
{
	...
	if(coll->type & NIL_NODE ||
	   coll->val.coll.size == 0)
		return nil_node;
	if(coll->type & LIST)
		return link_node(coll->val.coll[coll->val.coll.size - index - 1]]);
	else
		return link_node(coll->val.coll[index]]);
}

/*
	Allocate nodes
*/
static Node *malloc_coll(Node *coll, long size)
{
	ASSERT(coll, "malloc_coll : null pointer");
	ASSERT_TYPE(coll, LIST | ARRAY | MAP | SET, "alloc_coll : bad type %s", str_type(type));
...
/*
	Reallocate nodes
*/
static Node *realloc_coll(Node *colllong size)
{
	ASSERT(coll, "realloc_coll : null pointer");
	ASSERT_TYPE(coll, LIST | ARRAY | MAP | SET, "realloc_coll : bad type %s", str_type(type));
...
{
	ASSERT_TYPE(LIST | ARRAY | MAP | SET, "new_empty_coll : bad type %s", str_type(type));
